...
async function setup() {
  ...
  const receiver = await createReceiver({ output, token: "secret", identity,
    documentJson: "{}", width: 1, height: 1, port: 0 });
...
test("persists a valid PNG result and ignores attempts to complete twice", async () => {
  ...
  expect(JSON.parse(await readFile(join(output, "result.json"), "utf8"))).toMatchObject({ rootNodeId: "1:2" });
  const events = (await readdir(output)).filter(name => name.startsWith("event-"));
  expect(events).toEqual(["event-0001.json", "event-0002.json", "event-0003.json", "event-0004.json"]);
});
...
function resultMessage(message: Record<string, unknown>) {
  return { ...message, taskId: "run-1:geometry", documentJson: "{}",

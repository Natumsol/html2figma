...
export async function writeReport(output: string, summary: Record<string, unknown>) {
  ...
  const rows = cases.map(entry => {
    ...
    return `<tr><td>${name}</td><td>${source}</td><td>${escapeHtml(result?.status ?? "unexecuted")}</td><td>${escapeHtml(entry.documentSha256)}</td><td>${entry.maxDiffPixelRatio}</td><td>${escapeHtml(warnings)}</td><td>${image("browser")}</td><td>${result ? image("figma") : ""}</td><td>${result?.visualError ? image("diff") : ""}</td><td>${link}</td></tr>`;
  }).join("\n");
...
<h1>${cases.length} cases: ${escapeHtml(String(summary.status))}</h1><p>Real plugin API acceptance. Created nodes are retained. Browser-only UI tests are separate.</p>

...
figma.ui.onmessage = async (value: unknown) => {
  try {
    ...
    if (message.type !== "execute" || !readyState || started) return;
    started = true;
    const task = record(message.task);
    if (task.type !== "render-geometry" || task.taskId !== `${config.identity.runId}:geometry` ||
        task.documentJson !== config.documentJson) throw new Error("Unexpected render task/input");
    assertTarget();
    ...
    area.x = x; area.y = 100; area.resize(900, 600); area.fills = []; area.clipsContent = false;
    send("area", { taskId: task.taskId, areaId: area.id });
    const result = await render(document, { parent: area, x: 0, y: 0, loadFonts: true });
    ...
    send("result", { taskId: task.taskId, documentJson: JSON.stringify(document),
